...
export async function GET() {
  try {
    ...
    const scope = process.env.SCOPE || "https://api.businesscentral.dynamics.com/.default";
    ...
    if (!result.access_token) {
      return NextResponse.json({ error: "Invalid token response", details: result }, { status: 401 });
    }

    const expiresAt = Date.now() + result.expires_in * 1000 - 60_000; // minus 60s buffer

    // Save to cache
    fs.writeFileSync(cacheFile, JSON.stringify({ access_token: result.access_token, expires_at: expiresAt }, null, 2));
