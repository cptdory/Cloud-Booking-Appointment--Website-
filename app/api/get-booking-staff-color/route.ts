...
export async function POST(req: Request) {
  try {
    ...
    const { _BookingSetupCode, _BookingParameterId, _BookingParameterValueId } = body;

    if (!_BookingSetupCode || !_BookingParameterId || !_BookingParameterValueId) {
    ...
    if (!res.ok) {
      ...
      throw new Error(data?.error?.message || "Failed to get staff color");
    }

    return NextResponse.json(data);
  } catch (err: any) {
    console.error("POST /api/get-booking-staff-color failed:", err);
