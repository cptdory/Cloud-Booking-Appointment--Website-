...
export async function POST(req: Request) {
  try {
    ...
    const url = `https://api.businesscentral.dynamics.com/v2.0/${tenantId}/${environment}/ODataV4/BookingAppointment_UpdateBookingEntryStaffTimeOff?Company=${encodeURIComponent(company)}`;
...
if (!res.ok) {
  ...
  throw new Error(data?.error?.message || "Failed to update time off value");
}

return NextResponse.json({ success: true, data });

  } catch (err: any) {
    console.error("POST /api/booking-staff-timeoff/update-booking-staff-timeoff failed:", err);
