...
export async function POST(req: Request) {
  try {
    ...
    const url = `https://api.businesscentral.dynamics.com/v2.0/${tenantId}/${environment}/ODataV4/BookingAppointment_GetCustomerBookingEntries?Company=${encodeURIComponent(company)}`;
    ...
    if (!res.ok) {
      ...
      throw new Error(data?.error?.message || "Failed to get customer booking entries");
    }

    return NextResponse.json({ success: true, data });
  } catch (err: any) {
    console.error("POST /api/booking-entry/get-customer-booking-entries failed:", err);
