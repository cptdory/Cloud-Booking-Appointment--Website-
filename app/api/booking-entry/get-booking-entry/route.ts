...
export async function POST(req: Request) {
  try {
    ...
    const url = `https://api.businesscentral.dynamics.com/v2.0/${tenantId}/${environment}/ODataV4/BookingAppointment_GetBookingEntry?Company=${encodeURIComponent(company)}`;
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        _BookingEntryNo: _BookingEntryNo,
      }),
    });
    ...
    console.error("POST /api/booking-entry/get-booking-entry failed:", err);
