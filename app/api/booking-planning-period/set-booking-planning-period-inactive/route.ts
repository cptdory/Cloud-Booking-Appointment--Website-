...
export async function POST(req: Request) {
    try {
        ...
        const result = await bookingPlanningPeriodService.setBookingPlanningPeriodInactive(bookingSetupCode, code);
        const parsed = JSON.parse(result?.value || "null");
        return NextResponse.json(parsed);
    } catch (err: any) {
        return NextResponse.json(
            { error: err?.response?.data?.error?.message || "Failed to set booking planning period inactive", },
