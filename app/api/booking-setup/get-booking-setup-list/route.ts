...
async function fetchBusinessCentralData(
  ...
  if ((res.status === 401 || res.status === 403) && retry) {
    ...
    return await fetchBusinessCentralData(accessToken, body, false);
  }
...
// ===== GET - Fetch all booking setups (for branches) =====
export async function GET() {
  try {
    console.log("GET /api/booking-setup/get-booking-setup-list called");
...
// ===== POST - Fetch booking setups with parameters (for time slots) =====
export async function POST(req: NextRequest) {
  try {
    console.log("POST /api/booking-setup/get-booking-setup-list called");
    ...
    const data = await fetchBusinessCentralData(accessToken, body);
    console.log("Data fetched:", data);
    ...
    return NextResponse.json(
      { error: "Failed to fetch booking data", message: err.message },
      { status: 500 }
    );
  }
}