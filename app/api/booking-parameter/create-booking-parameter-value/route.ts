...
export async function POST(req: Request) {
  try {
    ...
    const url = `https://api.businesscentral.dynamics.com/v2.0/${tenantId}/${environment}/ODataV4/BookingAppointment_CreateBookingParameterValue?Company=${encodeURIComponent(company)}`;
...
if (!res.ok) {
  ...
  throw new Error(data?.error?.message || "Failed to create booking parameter value");
}

return NextResponse.json({ success: true, data });

  } catch (err: any) {
    console.error("POST /api/booking-parameter/create-booking-parameter-value failed:", err);
