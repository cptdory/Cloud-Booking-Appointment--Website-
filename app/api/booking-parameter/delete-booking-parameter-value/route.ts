// app/api/booking-parameter/delete-booking-parameter-value/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    const url = `https://api.businesscentral.dynamics.com/v2.0/${tenantId}/${environment}/ODataV4/BookingAppointment_DeleteBookingParameterValue?Company=${encodeURIComponent(company)}`;
    ...
    if (!res.ok) {
      return NextResponse.json({ message: "BC delete failed", detail: text }, { status: 500 });
    }

    return NextResponse.json({ message: "Deleted", detail: text });
  } catch (err: any) {
