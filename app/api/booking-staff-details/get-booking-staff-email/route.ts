...
export async function POST(req: Request) {
    try {
        ...
        const result = await bookingStaffDetailsService.getBookingStaffEmail(bookingSetupCode, bookingParameterId, bookingParameterValueId,);
        const parsed = JSON.parse(result?.value || "null");
        return NextResponse.json(parsed);
    } catch (err: any) {
        return NextResponse.json(
            { error: err?.response?.data?.error?.message || "Failed to get staff email", },
