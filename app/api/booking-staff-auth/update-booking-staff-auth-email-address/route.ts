...
export async function POST(req: Request) {
  try {
    ...
    const { _BookingSetupCode, _BookingParameterId, _BookingParameterValueId, _StaffEmail } = body;
    ...
    // 
    const url = `https://api.businesscentral.dynamics.com/v2.0/${tenantId}/${environment}/ODataV4/BookingAppointment_UpdateBookingStaffAuthEmailAddress?Company=${encodeURIComponent(company)}`;
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ 
        error: "Failed to update customer email address",
    ...
    console.error("POST /api/booking-staff-auth/update-booking-staff-auth-email-address failed:", err);
    return NextResponse.json({ 
